...
dir_img = Path("./data/s2_data/img/")
dir_mask = Path("./data/s2_data/cls/")
...
def train_model(
    ...
    if unknown_class_index is not None:
        weight = torch.ones((model.n_classes,))
        weight[unknown_class_index] = 0
        weight = weight.to(
            device=device,
            dtype=torch.float32,
        )
...
def get_args():
    ...
    parser.add_argument(
        "--unknown", "-u", type=int, default=None, help="Index of unknown class"
    )
...
if __name__ == "__main__":
    ...
    try:
        train_model(
            ...
            amp=args.amp,
            unknown_class_index=args.unknown,
        )
